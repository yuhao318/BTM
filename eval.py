...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    if config.resume:
        if os.path.isfile(config.resume):
            ...
            msg =model.load_state_dict(checkpoint['state_dict_model'], strict = False)
            logger.info(msg)

            msg =classifier.load_state_dict(checkpoint['state_dict_classifier'], strict = False)
            logger.info(msg)

            if config.dataset == 'places':
                msg = block.load_state_dict(checkpoint['state_dict_block'])
                logger.info(msg)

            if config.mode == 'stage2':
                msg = lws_model.load_state_dict(checkpoint['state_dict_lws_model'])
                logger.info(msg)
    ...
    # val_loader = dataset.eval_bl
    # val_loader = dataset.eval_train
...
def validate(val_loader, model, classifier, lws_model, criterion, config, logger, block=None):
    ...
    with torch.no_grad():
        ...
        print((acc_classes))
        print(len(acc_classes))
        acc_classes[acc_classes == 0] = 1e-3
        print("Hmean ", round(scipy.stats.hmean(acc_classes.cpu())*100,2))
        print("Gmean ", round(scipy.stats.gmean(acc_classes.cpu())*100,2))
        print(min(acc_classes))
        ...
        logger.info('* Acc@1 {top1.avg:.3f}% Acc@5 {top5.avg:.3f}% HAcc {head_acc:.3f}% MAcc {med_acc:.3f}% TAcc {tail_acc:.3f}% Loss {losses.avg:.3f}.'.format(top1=top1, top5=top5, head_acc=head_acc, med_acc=med_acc, tail_acc=tail_acc, losses=losses))
