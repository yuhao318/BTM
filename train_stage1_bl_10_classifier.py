...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    if config.resume:
        if os.path.isfile(config.resume):
            ...
            msg = model.load_state_dict(checkpoint['state_dict_model'])
            logger.info(msg)
    ...
    train_loader = dataset.bl_train_10_0_instance
    val_loader = dataset.eval
    if config.distributed:
        train_sampler = dataset.bl_dist_sampler
    ...
    if config.dataset == 'places':
        ...
        # optimizer = torch.optim.SGD(block.parameters(), config.lr,
        #                             momentum=config.momentum,
        #                             weight_decay=config.weight_decay)
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, config, logger, block=None):
    ...
    logger.info('* Acc@1 {top1.avg:.3f}% Acc@5 {top5.avg:.3f}% Loss {losses.avg:.3f}.'.format(top1=top1, top5=top5, losses=losses))
...
def validate(val_loader, model, classifier, criterion, config, logger, block=None):
    ...
    with torch.no_grad():
        ...
        print(scipy.stats.hmean(acc_classes.cpu()))
        print(scipy.stats.gmean(acc_classes.cpu()))
