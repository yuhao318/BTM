...
from methods import LabelAwareSmoothing, LearnableWeightScaling, ReweightedGMLLogAfterMean
...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    if config.resume:
        if os.path.isfile(config.resume):
            ...
            logger.info(msg)
            msg = classifier.load_state_dict(checkpoint['state_dict_classifier'])
            logger.info(msg)
    ...
    # criterion = ReweightedGMLLogAfterMean(num_class_list=cls_num_list, p=1.0)
